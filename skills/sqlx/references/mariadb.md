...
MariaDB speaks the MySQL protocol, so `--type mariadb` uses the MySQL worker and a `mysql` datasource reaches the same server. Official links target the current MariaDB documentation; check the connected server version first and select the matching documentation version when needed. An agent with web tools can follow each link; otherwise use the bundled recipe without claiming that the current official page was checked.

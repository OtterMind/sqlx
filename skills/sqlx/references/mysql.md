...
Official links target MySQL 8.4. Check the connected server version first and select the matching documentation version when needed. An agent with web tools can follow each link; otherwise use the bundled recipe without claiming that the current official page was checked.

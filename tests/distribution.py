...
def check_skill_source():
    ...
    databases=('mysql','mariadb','tidb','greatsql','oceanbase','postgresql','cockroachdb','yugabytedb','opengauss','oracle','sqlserver','clickhouse','trino','starrocks','doris','tdengine')
...
def check_release_contract():
    ...
    with tempfile.TemporaryDirectory(prefix='sqlx-manifest-') as tmp:
        directory=Path(tmp)
        shared={f'{kind}:any':{'version':'0.1.11'} for kind in once}
        (directory/'metadata-shared.json').write_text(json.dumps(shared))
        for platform in manifest.PLATFORMS:
            (directory/f'metadata-{platform}.json').write_text(
                json.dumps({f'{kind}:{platform}':{'version':'0.1.11'} for kind in per_platform}))
        def run(*extra):
        ...
        assert (directory/'SHA256SUMS').is_file() and (directory/'release-version.txt').read_text()=='0.1.11\n'
        (directory/'metadata-extra.json').write_text(json.dumps({'duckdb:any':{'version':'0.1.11'}}))
        try:
            run()

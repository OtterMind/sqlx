...
def exercise(cli):
    with tempfile.TemporaryDirectory(prefix='sqlx-distribution-') as tmp:
        ...
        def package(version,text,entries=None):
            ...
            return dict(version=version,url=f'{base}/{asset.name}',sha256=hashlib.sha256(asset.read_bytes()).hexdigest(),archive='zip',entrypoint='SKILL.md',cli_compat='>=0.1.0, <0.2.0',protocol_version=1)

...
def main():
    p=argparse.ArgumentParser();p.add_argument('--platform',required=True);p.add_argument('--version',default='0.2.0');p.add_argument('--bin-dir',type=Path,default=ROOT/'target/release');p.add_argument('--output',type=Path,default=ROOT/'dist');a=p.parse_args()
    ...
    for kind,binary in [('cli','sqlx'),('mysql','sqlx-driver-mysql'),('postgres','sqlx-driver-postgres'),('ui','sqlx-ui')]:
        ...
        records[f'{kind}:{a.platform}']=dict(version=a.version,url=f'https://github.com/OtterMind/sqlx/releases/download/v{a.version}/{asset.name}',sha256=digest,archive='zip',entrypoint=binary+suffix,cli_compat='>=0.2.0, <0.3.0',protocol_version=1)

...
def main():
    p=argparse.ArgumentParser();p.add_argument('--version',default='0.1.1');p.add_argument('--output',type=Path,default=ROOT/'dist');a=p.parse_args();a.output.mkdir(parents=True,exist_ok=True)
    ...
    def add(name,entries,entrypoint):
        ...
        records[f'{name}:any']=dict(version=a.version,url=base+path.name,sha256=hashlib.sha256(path.read_bytes()).hexdigest(),archive='zip',entrypoint=entrypoint,cli_compat='>=0.1.1, <0.2.0',protocol_version=1)
    ...
    plugin=ROOT/'ui/dist';add('ui-default',{str(f.relative_to(plugin)).replace('\\','/'):f.read_bytes() for f in plugin.rglob('*') if f.is_file()},'ui-plugin.json')
    vendors={

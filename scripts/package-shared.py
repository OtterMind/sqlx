...
def main():
    p=argparse.ArgumentParser();p.add_argument('--version',default='0.1.9');p.add_argument('--output',type=Path,default=ROOT/'dist');a=p.parse_args();a.output.mkdir(parents=True,exist_ok=True)
    ...
    def add(name,entries,entrypoint,version=None):
        ...
        # UI plugin is unchanged in 0.1.9 and keeps its 0.1.4 requirement.
